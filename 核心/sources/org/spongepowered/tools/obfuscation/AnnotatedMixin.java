...
/* loaded from: 核心-mc1.12.2-0.0.2.jar:org/spongepowered/tools/obfuscation/AnnotatedMixin.class */
    ...
    private TypeHandle initTargets() {
        ...
        try {
            for (String softTarget : this.annotation.getList("targets")) {
                TypeHandle type2 = this.typeProvider.getTypeHandle(softTarget);
                if (!this.targets.contains(type2)) {
                    if (this.virtual) {
                        type2 = this.typeProvider.getSimulatedHandle(softTarget, this.mixin.asType());
                    } else if (type2 == null) {
                        printMessage(Diagnostic.Kind.ERROR, "Mixin target " + softTarget + " could not be found", this);
                        return null;
                    } else if (type2.isPublic()) {
                        SuppressedBy suppressedBy = type2.getPackage().isUnnamed() ? SuppressedBy.DEFAULT_PACKAGE : SuppressedBy.PUBLIC_TARGET;
                        printMessage(Diagnostic.Kind.WARNING, "Mixin target " + softTarget + " is public and must be specified in value", this, suppressedBy);
                        return null;
                    }
                    addSoftTarget(type2, softTarget);
                    if (primaryTarget == null) {
                        primaryTarget = type2;
                    }
                }
