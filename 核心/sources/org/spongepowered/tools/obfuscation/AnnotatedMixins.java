...
/* loaded from: 核心-mc1.12.2-0.0.2.jar:org/spongepowered/tools/obfuscation/AnnotatedMixins.class */
final class AnnotatedMixins implements IMixinAnnotationProcessor, ITokenProvider, ITypeHandleProvider, IJavadocProvider {
    ...
    private AnnotatedMixins(ProcessingEnvironment processingEnv) {
        ...
        String pluginVersion = checkPluginVersion(getOption(SupportedOptions.PLUGIN_VERSION));
        String pluginVersionString = pluginVersion != null ? String.format(" (MixinGradle Version=%s)", pluginVersion) : "";
        printMessage(Diagnostic.Kind.NOTE, "SpongePowered MIXIN Annotation Processor Version=0.8.2" + pluginVersionString);
    ...
    private String checkPluginVersion(String version) {
        ...
        VersionNumber recommendedVersion = VersionNumber.parse(RECOMMENDED_MIXINGRADLE_VERSION);
        if (pluginVersion.compareTo(recommendedVersion) < 0) {
            printMessage(Diagnostic.Kind.WARNING, String.format("MixinGradle version %s is out of date. Update to the recommended version %s", pluginVersion, recommendedVersion));
        }
